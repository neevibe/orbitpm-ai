...
export default function ReportsPage() {
  ...
  const reports = [
    { title: 'Executive Summary', desc: 'Full portfolio overview with KPIs and department breakdown for leadership', icon: BarChart3, color: 'text-indigo-500', bg: 'bg-indigo-50', border: 'border-indigo-100', stats: `${projects.length} projects`, type: 'Executive_Summary' },
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="space-y-2.5">
        {reports.map(r => (
          <div key={r.type} className="glass-card p-4 hover:border-indigo-200 transition-all">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => handleExport(r.type)} disabled={exporting === r.type} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-medium transition-all ${
                success === r.type ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-white text-[#475569] border border-[#e2e8f0] hover:bg-[#f8fafc] hover:border-indigo-200'
              }`}>
