...
import { projects, departments, risks } from '../src/lib/mock-data';
...
async function sync() {
  ...
  const projectInserts = projects.map(p => ({
    ...
    target_date: parseDate(p.targetDate),
    ...
    archived_at: p.archivedAt ? new Date(p.archivedAt).toISOString() : null
  }));
  ...
  // 5. Sync Risks
  console.log(`Syncing ${risks.length} risks...`);
